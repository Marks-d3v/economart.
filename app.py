...
if not st.session_state.logado:
    with st.container():
        ...
        if st.button("🚀 ACESSAR MEU ECONOMART"):
            if nome and email:
    ...
    with aba2:
        ...
        # Editar Mercados
        with st.expander("🏢 Editar Nome dos Mercados"):
            for i in range(3):
        ...
        with st.expander("📝 Cadastrar/Editar Produtos"):
            m_sel = st.selectbox("Mercado", st.session_state.mercados)
            topicos = list(st.session_state.dados[m_sel].keys())
            t_sel = st.selectbox("Tópico", topicos + ["➕ Novo Tópico"])
            
            if t_sel == "➕ Novo Tópico":
                novo_t = st.text_input("Nome do Tópico")
                if st.button("Criar"):
